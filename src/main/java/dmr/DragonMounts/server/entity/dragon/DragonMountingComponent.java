...
abstract class DragonMountingComponent extends DragonOwnershipComponent {
    ...
    // BỔ SUNG: Cho phép tối đa 2 hành khách (1 người lái, 1 người ngồi sau)
    @Override
    ...
    public void positionRider(Entity passenger, Entity.MoveFunction callback) {
        ...
        if (riddenByEntity != null) {
            ...
            if (!customRidingPos) {
                ...
                // BỔ SUNG: Tính toán khoảng cách thụt lùi cho hành khách thứ 2
                double offsetX = 0;
                double offsetZ = 0;
                if (this.getPassengers().indexOf(passenger) == 1) { // Nếu là người thứ 2
                    float yBodyRotRad = this.yBodyRot * ((float)Math.PI / 180F);
                    // Lùi về sau khoảng 1.2 block (có thể tinh chỉnh con số này cho phù hợp với model rồng)
                    offsetX = Math.sin(yBodyRotRad) * 1.2; 
                    offsetZ = -Math.cos(yBodyRotRad) * 1.2;
                }

                Vec3 riderPos = new Vec3(vec3.x - vec31.x + offsetX, vec3.y - vec31.y, vec3.z - vec31.z + offsetZ);
